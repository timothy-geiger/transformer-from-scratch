...
def get_ds(config):
    ...
    train_ds_raw, val_ds_raw = \
        random_split(ds_raw, [train_ds_size, val_ds_size])
...
def train_model(config):
    ...
    for epoch in range(initial_epoch, config['num_epochs']):
        batch_iterator = tqdm(
            train_dataloader, desc=f'Processsing epoch {epoch:02d}')
        ...
        # save the model at the enc of every epoch
        mode_filename = get_weights_file_path(config, f'{epoch:02d}')
        torch.save({
            'epoch': epoch,
